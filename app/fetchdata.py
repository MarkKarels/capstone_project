...
def fetch_game_data(week_number, team_alias):
    ...
    print(f"Play-by-play data fetched and saved to database for week {week_number}, team {team_alias}")
...
def get_team_alias(full_team_name):
    ...
    return next((alias for alias, name in team_mapping.items() if name == full_team_name), None)
